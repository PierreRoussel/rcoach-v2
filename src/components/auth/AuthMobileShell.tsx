...
export function AuthMobileShell({
  ...
  return (
    <div className="relative flex min-h-svh flex-col overflow-hidden bg-gradient-hero-animated">
      ...
      <div className="relative flex flex-1 flex-col px-4 pb-2 pt-[max(1rem,env(safe-area-inset-top))]">
        <AuthMarketingHero variant={variant} className="flex-1 justify-center py-2" />
      </div>

      <div
        className={cn(
          'relative rounded-t-[1.75rem] border border-border/60 bg-background/95 px-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-5 shadow-[0_-12px_40px_rgba(0,0,0,0.08)] backdrop-blur-md',
          className,
        )}
      >
